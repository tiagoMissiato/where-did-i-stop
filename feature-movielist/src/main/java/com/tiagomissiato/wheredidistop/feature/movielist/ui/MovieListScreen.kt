...
fun MovieListScreen(modifier: Modifier = Modifier, viewModel: MovieListViewModel = hiltViewModel()) {
    ...
    val items by produceState<MovieListUiState>(
    ...
    if (items is Success) {
        MovieListScreen(
            ...
            onSave = { name -> viewModel.addMovieList(name) },
            modifier = modifier
        )
    }
}
...
internal fun MovieListScreen(
    ...
    Column(modifier) {
        var nameMovieList by remember { mutableStateOf("Compose") }
        Row(
...
private fun DefaultPreview() {
    MyApplicationTheme {
        MovieListScreen(listOf("Compose", "Room", "Kotlin"), onSave = {})
    }
}
